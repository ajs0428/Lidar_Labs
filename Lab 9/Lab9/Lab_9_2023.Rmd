...
[Lab 9 Submission Quiz](https://canvas.uw.edu/courses/1633883/quizzes/1863823)
...
This lab is all about Spaceborn lidar. We have covered aerial lidar, and terrestrial lidar, now we will talk about spaceborne lidar scanning (SLS).
...
Check out rGEDI here: https://github.com/carlos-alberto-silva/rGEDI
...
plot(wfb2, polygon = TRUE,  lwd = 2, col = "tan", xlab= "Waveform Amplitude", ylab = "Elevation(m)", main = "Barren")
```


```{r eval=FALSE}

wfb2 <- getLevel1BWF(gedilevel1b, shot_number = "72230300300100204") # fill in here
plot(wfb2, polygon = TRUE, lwd = 2, col = "forestgreen", xlab= "Waveform Amplitude", ylab = "Elevation(m)", main = "Forested")

wfb3 <- getLevel1BWF(gedilevel1b, shot_number = "72230300300100165") #fill in here
plot(wfb3, polygon = TRUE, lwd = 2, col = "lightgreen", xlab= "Waveform Amplitude", ylab = "Elevation(m)", main = "Light Veg")
```
...
### Part 5: Level 2B: Plant Area Index (PAI)
...
0.07273017 PAI for Barren
4.098562 PAI for Forested
0.04652207 PAI Light Veg
...
<!-- **`r colorize("QUESTION 1: Select a different beam. Include a screen shot of the beam with imagery base map and identify the beam in the image. Include screenshots of the PAI profile with and without the elevation. Caption the images describing what PAI is.", "red")`** -->
...
GEDI_DTM$elevation_bin0 <- GEDI_DTM$elevation_bin0*3.28
```


Now we can plot the elevation from GEDI and the elevation from the DTM together
```{r cache=TRUE}
plot(GEDI_DTM$elevation_bin0, GEDI_DTM$pack_forest_2013_dtm_1, xlim = c(0,2300), ylim = c(0, 2000))
```
