setwd("/Users/Anthony/OneDrive - UW/University of Washington/Teaching/SEFS433_Lidar/Labs/Lab 10/Lab_10_2023/")
library(lidR)
library(mapview)
library(BiocManager)
#BiocManager::install("EBImage") # run this if EBImage does not come up
library(EBImage)
library(rGEDI)
library(terra)
library(sf)
...
cr
PFTLS <- readLAS('LAB10_Data/LAB10_Data/2019-08-18 Pack Forest TMLS.laz')
HMTLS <- readTLSLAS('LAB10_Data_2023/2019-08-22 Hall of Mosses TMLS.laz')
PFALS <- readLAS('LAB10_Data_2023/WA_031_rp.laz')
HMALS <- readLAS('LAB10_Data_2023/q47123g8201.laz')
crs(PFTLS) <- NEWCRS
crs(HMTLS) <- NEWCRS
crs(PFALS) <- NEWCRS
crs(HMALS) <- NEWCRS
...
writeLAS(PFALSc,file = "LAB10_Data_2023/out/PFALSc.laz")
writeLAS(PFTLSc,file = "LAB10_Data_2023/out/PFTLSc.laz")
writeLAS(HMALSc,file = "LAB10_Data_2023/out/HMALSc.laz")
writeLAS(HMTLSc,file = "LAB10_Data_2023/out/HMTLSc.laz")
...
#PFTLS_tops <- locate_trees(PFTLScN, algorithm = lmf(ws = 3.28))
...
PFALS <- readLAS('LAB10_Data_2023/WA_031_rp.laz')
PFALS2 <- readLAS('LAB10_Data_2023/WA_038_rp.laz')
ONPALS <- readLAS('LAB10_Data_2023/q47123g8206.laz')
ONPALS2 <- readLAS('LAB10_Data_2023/q47123g8201.laz')
PF_DTM <- rast('LAB10_Data_2023/pack_forest_2013_dtm_1.tif')
ONP_DTM <- rast('LAB10_Data_2023/hoh_2013_dtm_4.tif')
ONP_DSM <- rast ('LAB10_Data_2023/hoh_2013_dsm_4.tif')
crs(PFALS) <- NEWCRS
crs(ONPALS) <- NEWCRS
...
#####################################################################################
gedilevel1b<-readLevel1B(level1Bpath = file.path ("LAB10_Data_2023/HM_level1b_clip.h5"))
gedilevel2a<-readLevel2A(level2Apath = file.path ("LAB10_Data_2023/HM_level2a_clip.h5"))
gedilevel2b<-readLevel2B(level2Bpath = file.path ("LAB10_Data_2023/HM_level2b_clip.h5"))
...
write_sf(level2AM_sf, ("out/GEDI_2A_EHM.shp"), overwrite=TRUE)
...
level2BVPM_sf<-st_as_sf(level2BVPM, coords = c("longitude_bin0", "latitude_bin0"), crs = 4326)

# Exporting GEDI Vegetation Profile Biophysical Variables as ESRI Shapefile
write_sf(level2BVPM_sf,("out/GEDI_2A_EHM.shp"), overwrite=TRUE)
...
writeRaster(rh100metrics$max,filename = ("out/rh100_max.tif")) 
...
crs(pai_metrics)
crs(pai_metrics) <- "EPSG:4326"
#location and file type for output
writeRaster(pai_metrics$max,filename =("out/PAI_max.tif")) 
...
level1bGeo_sf_prj <- st_transform(level1bGeo_sf, "EPSG:2927")
ONP_DTM_GEDI <- (terra::extract(ONP_DTM, level1bGeo_sf_prj, bind = TRUE))
ONP_DTM_GEDI <- na.omit(ONP_DTM_GEDI, "hoh_2013_dtm_4") #removing NA values 
mapview(st_as_sf(ONP_DTM_GEDI))


#check if there is a relationship between the GEDI and ALS elevation values
reg <- lm(ONP_DTM_GEDI$hoh_2013_dtm_4 ~ ONP_DTM_GEDI$elevation_bin0)
summary(reg)
plot(ONP_DTM_GEDI$elevation_bin0, ONP_DTM_GEDI$hoh_2013_dtm_4, 
     type = "p",
     xlab ="GEDI (m)", ylab="ALS (ft)", 
     main="GEDI (x) vs ALS (y) Elevation") 
abline(reg, col="blue") #regression line

#lets remove the weird GEDI elvevation values that are > 1200
ONP_DTM_GEDI_FILTER <- ONP_DTM_GEDI[ONP_DTM_GEDI$elevation_bin0 < 1200,]
ONP_DTM_GEDI_FILTER

#check if there is a relationship between the corrected GEDI and ALS elevation values
reg <- lm(ONP_DTM_GEDI_FILTER$hoh_2013_dtm_4 ~ ONP_DTM_GEDI_FILTER$elevation_bin0)
summary(reg)
plot(ONP_DTM_GEDI_FILTER$elevation_bin0, ONP_DTM_GEDI_FILTER$hoh_2013_dtm_4, 
     xlab="GEDI (m)", ylab="ALS (ft)", 
     main="NEW GEDI (x) vs ALS (y) Elevation") 
abline(reg, col="blue") #regression line
...
level2AGeo_sf_prj <- st_transform(level2AM_sf, "EPSG:2927")
ONP_CHM_GEDI<- (terra::extract(ONPCHM, level2AGeo_sf_prj, bind = TRUE))
ONP_CHM_GEDI_FILTER <- ONP_CHM_GEDI[is.na(ONP_CHM_GEDI$hoh_2013_dsm_4),] #removing NA values 
...
#lets remove the weird GEDI elvevation values that are > 1200
ONP_CHM_GEDI_FILTER <- ONP_CHM_GEDI[ONP_CHM_GEDI$rh100 > 0,]
ONP_CHM_GEDI_FILTER

reg <- lm(ONP_CHM_GEDI_FILTER$hoh_2013_dsm_4 ~ ONP_CHM_GEDI_FILTER$rh100)
summary(reg)
plot(ONP_CHM_GEDI_FILTER$rh100, ONP_CHM_GEDI_FILTER$hoh_2013_dsm_4 , xlab="GEDI rh100 (m)", ylab="ALS CHM (ft)", 
     main="Veg Height") 
abline(reg, col="blue")
