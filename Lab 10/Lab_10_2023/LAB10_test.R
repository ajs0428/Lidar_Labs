setwd("/Users/Anthony/OneDrive - UW/University of Washington/Teaching/SEFS433_Lidar/Labs/Lab 10/Lab_10_2023/LAB10_Data_2023/")
library(lidR)
library(mapview)
library(BiocManager)
...
writeLAS(PFALSc,file = "out/PFALSc.laz")
writeLAS(PFTLSc,file = "out/PFTLSc.laz")
writeLAS(HMALSc,file = "out/HMALSc.laz")
writeLAS(HMTLSc,file = "out/HMTLSc.laz")

mycsf <- csf(FALSE, 0.2, 0.2, rigidness = 3)
mypmf <- pmf(ws = 0.5, th = 0.1)
PFTLSc <- classify_ground(PFTLSc, mypmf)
PFDTM1 <-rasterzize_terrain(PFTLSc, res = 1, algorithm = knnidw())
plot_dtm3d(PFDTM1)
...
ONPALS2 <- readLAS('q47123g8201.laz')
PF_DTM <- rast('pack_forest_2013_dtm_1.tif')
ONP_DTM <- rast('hoh_2013_dtm_4.tif')
ONP_DSM <- rast ('hoh_2013_dsm_4.tif')
crs(PFALS) <- NEWCRS
crs(ONPALS) <- NEWCRS
...
gedilevel2b<-readLevel2B(level2Bpath = file.path ("HM_level2b_clip.h5"))
...
writeVector(ONP_DTM_GEDI_FILTER, filename = "out/ONP_DTM_GEDI_FILTER.shp")

#you can subtract the DTM from the DSM for an easy Canopy Height Model
ONPCHM <- ONP_DSM-ONP_DTM
plot(ONPCHM)

GEDIrh100 <- level2AM_sf$rh100
GEDIrh100 <- GEDIrh100[GEDIrh100 != 0] 

reg <- lm(ONP_DTM_GEDI$hoh_2013_dtm_4 ~ GEDIrh100)
summary(reg)
plot(GEDIrh100, ONP_DTM$hoh_2013_dtm_4, xlab="GEDI rh100 (m)", ylab="ALS CHM (ft)", 
     main="Veg Height") 
abline(reg, col="blue")
